...
import '../widgets/ldr/ldr_review_sheet.dart';
import '../widgets/logout_action.dart';
import '../widgets/notification_bell.dart';
import 'sec/ldr_wizard_screen.dart';
...
/// Full LDR case detail — shared by all three roles. Header/deceased/
/// applicant/documents/note-trail are identical everywhere; only the action
/// buttons at the bottom differ by role and status.
class LdrDetailScreen extends StatefulWidget {
  const LdrDetailScreen({super.key, required this.role, required this.caseId, required this.user});
  ...
  @override
  State<LdrDetailScreen> createState() => _LdrDetailScreenState();
}

class _LdrDetailScreenState extends State<LdrDetailScreen> {
  bool _loading = true;
  String? _error;
  DeathCase? _case;
  ...
  Future<void> _load() async {
    ...
    try {
      final deathCase = await DeathCaseService.instance.show(role: widget.role, id: widget.caseId);
      if (!mounted) return;
      setState(() {
        _case = deathCase;
        _loading = false;
  ...
  Future<void> _openReview() async {
    final updated = await showModalBottomSheet<DeathCase>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => LdrReviewSheet(role: widget.role, deathCase: _case!),
    );
  ...
  Future<void> _openCertificate() async {
    final updated = await showModalBottomSheet<DeathCase>(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => LdrCertificateSheet(caseId: widget.caseId),
    );
    if (updated != null) _load();
  }

  Future<void> _openResubmit() async {
    final updated = await Navigator.of(context).push(MaterialPageRoute(builder: (_) => LdrWizardScreen(existingCase: _case!)));
    if (updated != null) _load();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: Text(_case?.deathId ?? 'Application Details'), actions: const [NotificationBell(), LogoutAction()]),
      drawer: AppDrawer(role: widget.role, currentKey: 'ldr', user: widget.user),
      body: _loading
      ...
      children: [
        ...
        _sectionCard(
          ...
          child: Column(
            ...
            children: [
              _infoLine('Name', c.deceasedName),
              _infoLine('Gender', c.deceasedGender),
              _infoLine('Date of Death', c.dateOfDeath),
              if (c.delayYears != null) _infoLine('Delay', '${c.delayYears!.toStringAsFixed(1)} years'),
              if (c.deceasedCnic != null && c.deceasedCnic!.isNotEmpty) _infoLine('CNIC', c.deceasedCnic!),
              if (c.causeOfDeath != null && c.causeOfDeath!.isNotEmpty) _infoLine('Cause of Death', c.causeOfDeath!),
              if (c.placeOfDeath != null && c.placeOfDeath!.isNotEmpty) _infoLine('Place of Death', c.placeOfDeath!),
              if (c.burialPlace != null && c.burialPlace!.isNotEmpty) _infoLine('Burial Place', c.burialPlace!),
            ],
          ),
        ),
        const SizedBox(height: 16),
        ...
        if (c.courtDecree != null) ...[
          const SizedBox(height: 16),
          _sectionCard(
            title: 'Court Decree (Rule 13)',
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _infoLine('Decree No.', c.courtDecree!.decreeNo),
                _infoLine('Decree Date', c.courtDecree!.decreeDate),
                _infoLine('Court', c.courtDecree!.courtName),
              ],
            ),
          ),
        ],
        if (c.abroad != null) ...[
          const SizedBox(height: 16),
          _sectionCard(
            title: 'Death Abroad (Rule 15)',
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                _infoLine('Country of Death', c.abroad!.countryOfDeath),
                _infoLine('Passport No.', c.abroad!.passportNo),
              ],
            ),
          ),
        ],
        ...
        if (c.ddlgObservations != null && c.ddlgObservations!.isNotEmpty) ...[
          const SizedBox(height: 16),
          _observationsCard('DDLG Committee Observations', c.ddlgObservations!, c.ddlgOrderNo, AppColors.accent600),
        ],
  ...
  Widget _headerCard(DeathCase c) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(c.deceasedName, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: AppColors.ink)),
  ...
  Widget? _statusBanner(DeathCase c) {
    String? text;
    Color color = AppColors.info;
    if (c.status == 'PENDING_DDLG_APPROVAL') {
      text = 'ADLG reviewed this and forwarded it to the committee for a final decision.';
  ...
  List<Widget> _actionSection(DeathCase c) {
    ...
    if (widget.role == 'adlg' && c.status == 'FORWARDED') {
      ...
      widgets.add(_actionButton('Committee Decision', _openReview));
    } else if (widget.role == 'sec') {
      if (c.status == 'APPROVED') {
        widgets.add(_actionButton('📜 Issue Certificate & Lock File', _openCertificate));
      } else if (c.status == 'RETURNED') {
  ...
  Widget _timelineList(DeathCase c) {
    return Column(
      children: [
  ...
  Widget _docChip(BuildContext context, LdrDocument d) {
