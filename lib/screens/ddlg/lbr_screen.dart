...
/// District-wide Over-7-Years birth registration registry — DDLG never sees
/// 1-7 year cases (the API itself scopes them out), so there's no category
/// filter here, unlike the Secretary/ADLG screens.
class DdlgLbrScreen extends StatefulWidget {
  const DdlgLbrScreen({super.key, required this.user});

  final Map<String, dynamic> user;

  @override
  State<DdlgLbrScreen> createState() => _DdlgLbrScreenState();
}

class _DdlgLbrScreenState extends State<DdlgLbrScreen> {
  bool _loading = true;
  String? _error;
  List<LbrCase> _cases = [];
  String _search = '';
  ...
  Future<void> _load() async {
    ...
    try {
      final cases = await LbrService.instance.indexForDdlg(status: _statusFilter);
      if (!mounted) return;
      setState(() {
        _cases = cases;
        _loading = false;
  ...
  Future<void> _openDetail(LbrCase c) async {
    await Navigator.of(context).push(MaterialPageRoute(builder: (_) => LbrDetailScreen(role: 'ddlg', caseId: c.id, user: widget.user)));
    _load();
  }
  ...
  Future<void> _export() async {
    ...
    try {
      final file = await LbrService.instance.exportCases(role: 'ddlg', status: _statusFilter);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer: AppDrawer(role: 'ddlg', currentKey: 'lbr', user: widget.user),
      body: _loading
          ...
          : _error != null
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      Text('Full registry for your district — final approval for Over-7-Years cases', style: const TextStyle(fontSize: 11.5, color: AppColors.inkMuted)),
