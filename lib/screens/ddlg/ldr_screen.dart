...
const _kStatusOptions = [
  (null, 'All Statuses'),
  ('FORWARDED', 'With ADLG'),
  ('PENDING_DDLG_APPROVAL', 'Pending My Approval'),
  ('APPROVED', 'Approved'),
  ('REJECTED', 'Rejected'),
  ('RETURNED', 'Returned'),
...
/// District-wide death registration registry — unlike LBR, DDLG sees EVERY
/// category here (no server-side category restriction); the actionable
/// subset is just whatever's PENDING_DDLG_APPROVAL, which by construction
/// only ever happens for 1-7 and ABROAD cases ('7+' is always ADLG-final).
class DdlgLdrScreen extends StatefulWidget {
  const DdlgLdrScreen({super.key, required this.user});

  final Map<String, dynamic> user;

  @override
  State<DdlgLdrScreen> createState() => _DdlgLdrScreenState();
}

class _DdlgLdrScreenState extends State<DdlgLdrScreen> {
  bool _loading = true;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    try {
      final cases = await DeathCaseService.instance.indexForDdlg(status: _statusFilter);
      if (!mounted) return;
      setState(() {
        _cases = cases;
        _loading = false;
  ...
  Future<void> _openDetail(DeathCase c) async {
    await Navigator.of(context).push(MaterialPageRoute(builder: (_) => LdrDetailScreen(role: 'ddlg', caseId: c.id, user: widget.user)));
    _load();
  }
  ...
  Future<void> _export() async {
    ...
    try {
      final file = await DeathCaseService.instance.exportCases(role: 'ddlg', status: _statusFilter);
  ...
  Widget build(BuildContext context) {
    final pending = _cases.where((c) => c.status == 'PENDING_DDLG_APPROVAL').length;
    ...
    return Scaffold(
      ...
      drawer: AppDrawer(role: 'ddlg', currentKey: 'ldr', user: widget.user),
      body: _loading
          ...
          : _error != null
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      Text('Full registry for your district — committee decision for 1–7 year and Abroad cases', style: const TextStyle(fontSize: 11.5, color: AppColors.inkMuted)),
                      ...
                      if (pending > 0)
                        Container(
                          ...
                          child: Text('$pending awaiting your approval', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: AppColors.accent600)),
                        ),
                      TextField(
