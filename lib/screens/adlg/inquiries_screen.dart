...
/// Inquiry Requests — ADLG uploads a file with remarks, Super Admin drafts
/// and returns a formal report. Mirrors adlg/Inquiries.jsx.
class AdlgInquiriesScreen extends StatefulWidget {
  const AdlgInquiriesScreen({super.key, required this.user});

  final Map<String, dynamic> user;

  @override
  State<AdlgInquiriesScreen> createState() => _AdlgInquiriesScreenState();
}

class _AdlgInquiriesScreenState extends State<AdlgInquiriesScreen> {
  bool _loading = true;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    try {
      final inquiries = await InquiryService.instance.indexForAdlg();
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _openSubmit() async {
    final created = await showModalBottomSheet<Inquiry>(
      ...
      builder: (_) => const SubmitInquirySheet(role: 'adlg'),
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      drawer: AppDrawer(role: 'adlg', currentKey: 'inquiries', user: widget.user),
          ...
          : _error != null
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      const Text('Upload your inquiry file with remarks — Super Admin will draft the formal report.', style: TextStyle(fontSize: 11.5, color: AppColors.inkMuted)),
                          ...
                          child: Center(
                            child: Column(
                              children: [
                                ...
                                SizedBox(height: 10),
                                Text('No inquiries submitted yet', style: TextStyle(fontWeight: FontWeight.w700, color: AppColors.ink)),
                              ],
                            ),
                          ),
                        )
                      else
                        ..._inquiries.map((i) => _InquiryCard(i: i, showAdlg: false)),
                    ],
                  ),
                ),
    );
        ...
        children: [
          ...
          if (i.fileUrl != null || i.reportFileUrl != null) ...[
            ...
            Wrap(
              ...
              runSpacing: 8,
              children: [
                if (i.fileUrl != null) _fileChip(context, icon: Icons.attach_file_rounded, label: 'My File', url: i.fileUrl!),
