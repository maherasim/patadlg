...
/// Admin, mirrors adlg/Newsletters.jsx.
class AdlgNewslettersScreen extends StatefulWidget {
  const AdlgNewslettersScreen({super.key, required this.user});

  final Map<String, dynamic> user;

  @override
  State<AdlgNewslettersScreen> createState() => _AdlgNewslettersScreenState();
}

class _AdlgNewslettersScreenState extends State<AdlgNewslettersScreen> {
  bool _loading = true;
  String? _error;
  ...
  Future<void> _load() async {
    ...
    try {
      final newsletters = await NewsletterService.instance.indexForAdlg();
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _openRespond(Newsletter n) async {
    final ok = await showModalBottomSheet<bool>(
      ...
      builder: (_) => NewsletterRespondSheet(role: 'adlg', newsletter: n),
    );
    if (ok == true) _load();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      drawer: AppDrawer(role: 'adlg', currentKey: 'newsletters', user: widget.user),
      body: _loading
