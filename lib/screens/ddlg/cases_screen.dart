...
/// District-wide Divorce/Khula case registry — read-only oversight, matching
/// the web app: DDLG never acts on these cases (no review/decision endpoint
/// exists for this role), and there's no Excel export route for it either.
class DdlgCasesScreen extends StatefulWidget {
  const DdlgCasesScreen({super.key, required this.user});

  final Map<String, dynamic> user;

  @override
  State<DdlgCasesScreen> createState() => _DdlgCasesScreenState();
}

class _DdlgCasesScreenState extends State<DdlgCasesScreen> {
  bool _loading = true;
  String? _error;
  List<DvCase> _cases = [];
  String _search = '';
  ...
  Future<void> _load() async {
    ...
    try {
      final cases = await CaseService.instance.indexForDdlg(status: _statusFilter);
      if (!mounted) return;
      setState(() {
        _cases = cases;
        _loading = false;
  ...
  Future<void> _openDetail(DvCase c) async {
    await Navigator.of(context).push(MaterialPageRoute(builder: (_) => CaseDetailScreen(role: 'ddlg', caseId: c.id, user: widget.user)));
    _load();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Divorce/Khula Cases'), actions: const [NotificationBell(), LogoutAction()]),
      drawer: AppDrawer(role: 'ddlg', currentKey: 'cases', user: widget.user),
      body: _loading
          ...
          : _error != null
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      Text('Full registry for your district — view only', style: const TextStyle(fontSize: 11.5, color: AppColors.inkMuted)),
