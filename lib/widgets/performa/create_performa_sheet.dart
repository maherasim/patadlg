...
/// ADLG-only — publishes a new Performa (form-mode dynamic form, or
/// excel-mode template) to every secretary in the tehsil, immediately live
/// (there's no draft state on the web app this mirrors).
class CreatePerformaSheet extends StatefulWidget {
  const CreatePerformaSheet({super.key});

  @override
  State<CreatePerformaSheet> createState() => _CreatePerformaSheetState();
}

class _CreatePerformaSheetState extends State<CreatePerformaSheet> {
  final _titleController = TextEditingController();
  final _descriptionController = TextEditingController();
  String _mode = 'form';
  String _reportType = 'onetime';
  DateTime? _deadline;
  File? _excelTemplate;
  final List<_FieldRow> _fields = [_FieldRow()];
  ...
  Future<void> _submit() async {
    ...
    if (_mode == 'excel' && _excelTemplate == null) {
    ...
    final result = await PerformaService.instance.createPerforma(
        ...
        builder: (context, scrollController) => Container(
          ...
          child: ListView(
            ...
            children: [
              ...
              const Text('Publish Performa', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800, color: AppColors.ink)),
              const SizedBox(height: 4),
              const Text('Broadcast to every secretary in your tehsil.', style: TextStyle(fontSize: 12, color: AppColors.inkMuted)),
              const SizedBox(height: 18),
              _label('Title'),
