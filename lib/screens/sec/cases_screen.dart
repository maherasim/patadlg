...
class SecCasesScreen extends StatefulWidget {
  const SecCasesScreen({super.key, required this.user});

  final Map<String, dynamic> user;

  @override
  State<SecCasesScreen> createState() => _SecCasesScreenState();
}

class _SecCasesScreenState extends State<SecCasesScreen> {
  bool _loading = true;
  String? _error;
  List<DvCase> _cases = [];
  String _search = '';
  ...
  Future<void> _load() async {
    ...
    try {
      final cases = await CaseService.instance.indexForSecretary();
      if (!mounted) return;
      setState(() {
        _cases = cases;
        _loading = false;
  ...
  List<DvCase> get _filtered {
    ...
    return _cases.where((c) {
      return c.caseNo.toLowerCase().contains(q) || c.divorcerName.toLowerCase().contains(q) || c.respondentName.toLowerCase().contains(q);
    }).toList();
  }

  Future<void> _openNewCase() async {
    final created = await Navigator.of(context).push<DvCase>(MaterialPageRoute(builder: (_) => const NewCaseScreen()));
    if (created != null) _load();
  }

  Future<void> _openDetail(DvCase c) async {
    await Navigator.of(context).push(MaterialPageRoute(builder: (_) => CaseDetailScreen(role: 'sec', caseId: c.id, user: widget.user)));
    _load();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Cases'), actions: const [NotificationBell(), LogoutAction()]),
      drawer: AppDrawer(role: 'sec', currentKey: 'cases', user: widget.user),
          ...
          : _error != null
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      Text('${_cases.length} cases in your Union Council', style: const TextStyle(fontSize: 12.5, color: AppColors.inkMuted)),
                      const SizedBox(height: 12),
                      TextField(
                        onChanged: (v) => setState(() => _search = v),
                        decoration: const InputDecoration(hintText: 'Search case no., divorcer, respondent…', prefixIcon: Icon(Icons.search_rounded, size: 20)),
                      ),
